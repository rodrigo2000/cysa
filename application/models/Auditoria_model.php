...
class Auditoria_model extends MY_Model {
    ...
    function get_mis_auditorias($anio = NULL, $empleados_id = NULL, $status_id = NULL, $is_agrupado = TRUE) {
        ...
        $this->db->select($this->table_prefix . ".*")
                ->select("IF(CONVERT(a.auditorias_numero, UNSIGNED)>0,CONCAT(LPAD(a.auditorias_numero, 3, '0'), ' - ', IF(a.auditorias_segundo_periodo=1, '2', ''), `aa`.`auditorias_areas_siglas`, '/', `at`.`auditorias_tipos_siglas`, '/', LPAD(a.auditorias_numero, 3, '0'), '/', a.auditorias_anio), NULL) AS 'numero_auditoria'")
