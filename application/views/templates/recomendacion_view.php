...
<div id="<?= isset($rr, $rr['recomendaciones_id']) ? 'recomendacion_' . $rr['recomendaciones_id'] : 'nueva_recomendacion_' ?>" class="card XXXcard-inverse XXXcard-<?= $css[$status_id]; ?>">
    <form>
        ...
        <div class="card-block">
            ...
            <?php else: ?>
                <div class="text-2-html"><?= $rr['recomendaciones_descripcion']; ?></div>
            <?php endif; ?>
        </div>
        <?php if (isset($avance) && !empty($avance)): ?>
            <div class="card-block">
                <h4 class="card-title">
                    Avance
                ...
                <?php if ($etapa_auditoria < AUDITORIA_ETAPA_FIN): ?>
                    <textarea name="recomendaciones_avaces_descripcion[]" class="editor_html"><?= $avance['recomendaciones_avances_descripcion']; ?></textarea>
                    <input type="hidden" class="recomendaciones_avances_numero_revision" name="recomendaciones_avances_numero_revision[]" value="<?= $avance['recomendaciones_avances_numero_revision']; ?>">
                <?php else: ?>
                    <div class="font-barlow font-size-1rem"><?= $avance['recomendaciones_avances_descripcion']; ?></div>
                <?php endif; ?>
            </div>
            <div class="card-footer">
                <div class="row align-middle">
                    <div class="col-xs-12 col-sm-4 text-xs-center">
                        ...
                        <select id="recomendaciones_empleados_id" name="recomendaciones_empleados_id[]" class="form-control" <?= ($etapa_auditoria > AUDITORIA_ETAPA_REV1) ? 'disabled="disabled"' : ''; ?>>
                            <option value="0">SELECCIONE</option>
                            <?php foreach ($empleados_involucrados as $e): ?>
                                <option value="<?= $e['empleados_id']; ?>" <?= $e['empleados_id'] == $avance['recomendaciones_avances_empleados_id'] ? 'selected="selected"' : ''; ?>><?= $e['empleados_nombre_titulado_siglas']; ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-xs-12 col-sm-4 text-xs-center">
                        ...
                        <select id="recomendaciones_clasificaciones_id" name="recomendaciones_clasificaciones_id[]" class="form-control" <?= ($etapa_auditoria > AUDITORIA_ETAPA_REV1) ? 'disabled="disabled"' : ''; ?>>
                            <option value="0">SELECCIONE</option>
                            <?php foreach ($recomendaciones_clasificaciones as $rc): ?>
                                <option value="<?= $rc['recomendaciones_clasificaciones_id']; ?>" <?= $rc['recomendaciones_clasificaciones_id'] == $avance['recomendaciones_avances_recomendaciones_clasificaciones_id'] ? 'selected="selected"' : ''; ?>><?= $rc['recomendaciones_clasificaciones_nombre']; ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-xs-12 col-sm-4 text-xs-center">
                        ...
                        <select id="recomendaciones_status_id" name="recomendaciones_status_id[]" class="form-control" <?= ($etapa_auditoria > AUDITORIA_ETAPA_REV1) ? 'disabled="disabled"' : ''; ?>>
                            <option value="0">SELECCIONE</option>
                            <?php foreach ($recomendaciones_status as $rs): ?>
                                <option value="<?= $rs['recomendaciones_status_id']; ?>" <?= $rs['recomendaciones_status_id'] == $avance['recomendaciones_avances_recomendaciones_status_id'] ? 'selected="selected"' : ''; ?>><?= $rs['recomendaciones_status_nombre']; ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
            ...
            <div class="card-footer">
                <div class="row align-middle">
                    <div class="col-xs-12 col-sm-4 text-xs-center">
                        <b>Responsable:</b><br>
                        <?php $empleados_involucrados = $this->Auditoria_model->get_involucrados(); ?>
                        <select id="recomendaciones_empleados_id" name="recomendaciones_empleados_id[]" class="form-control" <?= ($etapa_auditoria > AUDITORIA_ETAPA_AP) ? 'disabled="disabled"' : ''; ?>>
                            <option value="0">SELECCIONE</option>
                            <?php foreach ($empleados_involucrados as $e): ?>
                    ...
                    <div class="col-xs-12 col-sm-4 text-xs-center">
                        <b>Clasificación:</b><br>
                        <?php $recomendaciones_clasificaciones = $this->Recomendaciones_clasificaciones_model->get_todos(); ?>
                        <select id="recomendaciones_clasificaciones_id" name="recomendaciones_clasificaciones_id[]" class="form-control" <?= ($etapa_auditoria > AUDITORIA_ETAPA_AP) ? 'disabled="disabled"' : ''; ?>>
                            <option value="0">SELECCIONE</option>
                            <?php foreach ($recomendaciones_clasificaciones as $rc): ?>
                    ...
                    <div class="col-xs-12 col-sm-4 text-xs-center">
                        <b>Status:</b><br>
                        <?php $recomendaciones_status = $this->Recomendaciones_status_model->get_todos(); ?>
                        <select id="recomendaciones_status_id" name="recomendaciones_status_id[]" class="form-control" <?= ($etapa_auditoria > AUDITORIA_ETAPA_AP) ? 'disabled="disabled"' : ''; ?>>
                            <option value="0">SELECCIONE</option>
                            <?php foreach ($recomendaciones_status as $rs): ?>
