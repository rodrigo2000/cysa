...
include_once('../../_funciones/configuracion_entorno.php');
...
define('OBSERVACION_NO_SOLVENTADA', 1);
define('OBSERVACION_SOLVENTADA', 2);
define('OBSERVACION_ATENDIDA', 4);
...
function get_etapas($idAuditoria, $idProceso = NULL, $audi = array()) {
    ...
    if (!empty($idProceso)) {
        ...
        if (!empty($audi['idAuditoriaOrigen'])) {
            $strSQL .= " AND etapas_codigo = " . ETAPA_RV1;
        }
        if ($audi['bSinObservacionAP'] == 1 && $audi['anio'] >= 2018) {
            $strSQL .= " AND etapas_codigo = " . ETAPA_AP;
        }
    ...
    if (!empty($idEtapa)) {
        ...
        if (mysql_num_rows($res) > 0) {
            ...
            foreach ($return as $index => $r) {
                ...
                } elseif ($r['etapas_codigo'] == ETAPA_AP && $isAuditoriaSinObservaciones && $audi['anio'] >= 2018) {
                ...
                if ($mostrarTarea) {
                    ...
                    if ($audi['anio'] == 2018 && in_array($return[$index]['tareas_nombre'], $activarTareasPor10Dias)) {
                        if (!empty($audi['fechaLectura'])) {
                            $dias = getDiasHabiles($audi['fechaLectura'], date("Y-m-d"));
                            if ($dias < 10) {
...
//        }
        if (!is_null($fechaProgramada) && $duracion != 0) {
            if ($duracion < 0) {
                ...
                $fechaProgramada = agregarDiasHabiles($fechaProgramada, $duracion);
            }
        }
    }
    if ($returnCampoReal) {
...
function get_timeline($idAuditoria = NULL) {
    ...
    $result = $dbCYSA->ejecutaQuery($strSQL);
    $r = mysql_fetch_assoc($result);
    if (!empty($r['idAuditoriaOrigen']) && $r['anio'] < 2018) {
        $select = array(
    ...
    $strSQL = "SELECT CONCAT(area,'/',tipo,'/',numero,'/',anio) AS nombreAuditoria, tipo, idEmpleado, bSinObservacionAP, fLimiteInfoRev1, fechaRecibeInfoRev1, fechaAprovacionRev1, statusAudit, fechaSelloOEA, anio, fechaLectura, fechaLecturaRev1, fechaLecturaRev2, " . implode(",", $select) . " FROM cat_auditoria WHERE idAuditoria = " . $idAuditoria . " LIMIT 1";
    $result = $dbCYSA->ejecutaQuery($strSQL);
